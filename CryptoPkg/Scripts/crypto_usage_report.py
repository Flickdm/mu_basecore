...
# These are excluded from source scanning because they contain wrapper
# definitions for every crypto function.
...
def _parse_library_subsection(lines: list, start: int) -> Tuple[Dict[str, str], int]:
    ...
    while i < len(lines):
        ...
        entry = line
        while "}" not in entry and i + 1 < len(lines):
            i += 1
            entry += lines[i].strip()
...
def is_crypto_provider(inf_path: str) -> bool:
    ...
    return any(marker in normalized for marker in CRYPTO_PROVIDER_MARKERS)
