...
"""
# TODO reimplement this with edk2toollib
...
def pkcs7_sign(buffer, signers: list, top_level_certificate_file=None, additional_cerificates=[]) -> rfc2315.SignedData:
    """
    ...
    for i, signer in enumerate(signers):
        ...
        signature_builder = signature_builder.add_signer(
            pkcs12_store.cert.certificate, pkcs12_store.key, hashes.SHA256())
    ...
    # TODOUG - this is a hack to get around the fact that the EDK2 implementation of PKCS7_signatureVerify
    return signature
...
class AuthenticatedVariable2(object):
    ...
    def sign_payload(self, signers, top_level_certificate, additional_cerificates):
        """
        ...
        self.pkcs7_signature = pkcs7_sign(
            self.signature_buffer, signers, top_level_certificate, additional_cerificates)

        self.signature = pkcs7_get_signed_data_structure(self.pkcs7_signature)
...
def create_authenticated_variable_v2(tm, name, guid, attributes, data_file, signers, top_level_certificate=None,
                                     additional_cerificates=[], output_dir='./'):
    """
    ...
    signature = auth_var.sign_payload(
        signers, top_level_certificate, additional_cerificates)
    if not signature:
        logger.error("Signature Failed")
        return

    signature_file = data_file + ".signature"
    if output_dir:
        filename = os.path.split(signature_file)[-1]
        signature_file = os.path.join(output_dir, filename)

    with open(signature_file, 'wb') as f:
        f.write(auth_var.pkcs7_signature)
...
def sign_variable(args):
    ...
    output_file, signature_file = create_authenticated_variable_v2(
        tm, args.name, args.guid, args.attributes, args.data_file, args.signers_info,
        args.top_level_certificate, args.additional_certificates, output_dir)
    ...
    if args.export_c_array:
        export_c_array(signature_file, output_dir, args.name, args.c_name)
...
def setup_sign_parser(subparsers):
    ...
    :returns: subparser
    """
    # Enabling debug will write the signature to a file
