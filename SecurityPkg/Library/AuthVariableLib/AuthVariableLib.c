...
//
// Crypto operations backing ECIT authenticated-variable capability reporting:
// the PKCS#7/CMS signer signature algorithms Pkcs7Verify accepts, plus the CMS
// content-digest (message-digest) algorithms used to pre-hash the update
// content when authorizing a time-based authenticated variable update.
// Reported as a single flat OID list.
//
