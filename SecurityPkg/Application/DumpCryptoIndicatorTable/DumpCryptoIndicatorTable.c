...
STATIC CONST ECIT_OID_NAME  mOidNames[] = {
  ...
  //
  // Digest (hash) algorithms. Kept in lockstep with mDigestOids[] below so
  // every digest the classifier recognizes also prints a friendly name.
  //
  { "1.3.14.3.2.26",           "sha1"                    },
  ...
  for (Ptr = TokenStart; ; Ptr++) {
    if ((*Ptr == ',') || (*Ptr == '\0')) {
      ...
      if (Length > 0) {
        ...
        if ((IsDigestOid (Oid) ? TRUE : FALSE) == WantDigest) {
          if (!HeaderPrinted) {
            ECIT_DUMP ("%a", Header);
            HeaderPrinted = TRUE;
          }
