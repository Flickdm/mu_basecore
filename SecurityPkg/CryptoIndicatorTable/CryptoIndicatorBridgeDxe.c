/** @file
  ...
  Drains the Standalone MM collector's accumulated ECIT records into the DXE
  collector before the table is sealed. Feature owners whose cryptographic work
  runs in MM (for example authenticated-variable servicing) register with the MM
  collector; this driver fetches those records via MM communication and submits
